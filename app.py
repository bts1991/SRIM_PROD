...
from request import RequestStock, RequestCurrentStockPrice
...
def updateShow():
    CoID = request.form['CoID']
    baseDate = request.form['baseDate']
    show_yn = request.form['show_yn']
    updateShowYn(CoID, baseDate, show_yn)

    return redirect('/')

@app.route("/updateCurrentStockPrice", methods=["POST"])
def updateCurrentStockPrice():
    ...
    CoIDList = main.distinct("CoID",{})
    for i in CoIDList:
        print("이름은?? "+i)
        currentPrice=RequestCurrentStockPrice(i)
        print("현재가는?? "+str(currentPrice))
        main.update_one({"CoID":i},{"$set" : {"currentPrice": currentPrice}})
...
def get():
    ...
    data = main.find().sort([("CoID",1),("baseDate",1)])
    data_list = list(data)
    result_list = []
    ...
    for i in range(len(data_list)):
        ...
        show_yn = '' if data_dic['show_yn'] == 'Y' else 'style=display:none'
        checked_yn = 'checked=checked' if data_dic['show_yn'] == 'Y' else ''
        ...
        result['show_yn'] = show_yn
        result['checked_yn'] = checked_yn
